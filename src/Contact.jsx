...
export default function Contact() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Contact response was not JSON:', responseText)
      }

      if (!response.ok) {
        throw new Error(result.message || response.statusText || 'Unable to send message')
      }

      setStatus({ submitting: false, message: result.message || 'Message sent successfully. We will contact you soon.', error: '' })
  ...
  <div className="w-full max-w-3xl rounded-[2rem] border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-[#0d1b2e]/80 p-8 shadow-sm transition-colors">
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <label className="block">
        ...
        <textarea
          ...
          required
          className="mt-2 w-full rounded-2xl border border-gray-300 px-4 py-3 text-sm text-gray-900 outline-none !bg-transparent transition-colors focus:border-cyber-green focus:ring-2 focus:ring-cyber-green/20 dark:border-gray-700 dark:bg-[#111827] dark:text-gray-100"
        />
      </label>
